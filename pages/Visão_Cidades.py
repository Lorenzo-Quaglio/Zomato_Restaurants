...
import inflection
import numpy as np
from utils.functions import country_name, create_type_price, color_name, rename_columns, cidade_com_mais_restaurantes_por_pais, top_10_cidades_melhor_avaliadas, top_10_cidades_piores_avaliadas, top_10_cidades_culinarias_distintas, quantidade_restaurantes_por_cidade
...
st.markdown('<br><h3 style="color: tomato;text-align: left;">🌆 VISÃO CIDADES</h3><br>', unsafe_allow_html=True)

with st.container():
    fig_1 = cidade_com_mais_restaurantes_por_pais(dc)
...
with st.container():
    ...
    with col_1:
        fig_2 = top_10_cidades_melhor_avaliadas(dc)
        st.plotly_chart(fig_2)       
            
    with col_2:
        fig_3 = top_10_cidades_piores_avaliadas(dc)
        st.plotly_chart(fig_3)  


with st.container():
    fig_4 = top_10_cidades_culinarias_distintas(dc)
    st.plotly_chart(fig_4)
    
with st.container():
    fig_5 = quantidade_restaurantes_por_cidade(dc)
